...
function getBaseURL(){var url=location.href;var baseURL=url.substring(0,url.indexOf('/',14));if(baseURL.indexOf('http://localhost')!=-1){var url=location.href;var pathname=location.pathname;var index1=url.indexOf(pathname);var index2=url.indexOf("/",index1+ 1);var baseLocalUrl=url.substr(0,index2);return baseLocalUrl+"/wishes/";}else{return baseURL+"/";}}
function selected(){var BaseURL=getBaseURL();$('#error-display').hide();var selected=$("#wishes-select option:selected").val();if(selected=='birthday'){$('div#bnames').show();}
if(selected!='birthday'){$('div#bnames').hide();}}
function validateAlpha(){$('#error-display').hide();$('#username').keypress(function(e){var regex=new RegExp("^[a-zA-Z0-9 ]+$");var str=String.fromCharCode(!e.charCode?e.which:e.charCode);if(regex.test(str)){return true;}
e.preventDefault();return false;});}
function create(){var BaseURL=getBaseURL();var error='0';var name='';var bname='';var bwords='';var cat='';name=$("#username").val();if(name==''){$('#error-display').html('Please Enter Your Name');$('#error-display').show();error='1';return false;}else{name=name.replace(/[^A-Za-z ]/g,"");name=name.replace(/ /g,'-');}
var selected=$("#wishes-select option:selected").val();if(selected=='0'){error='1';$('#error-display').html('Select Your Wishing Choise');$('#error-display').show();return false;}
if(selected=="birthday"){bname=$("#bname").val();bwords=$('#bwords').val();if(bname==''){$('#error-display').html('Enter Birthday Person Name');$('#error-display').show();error='1';return false;}else{bname=bname.replace(/[^A-Za-z ]/g,"");bname=bname.replace(/ /g,'-');window.location=BaseURL+'open/?n='+name+'/&b='+bname+'/';}}else{window.location=BaseURL+selected+'/?n='+name+'/';}
if(error=='0'){$.ajax({type:"POST",url:BaseURL+'create/',data:{"name":name,"selected":selected,"bname":bname,"message":bwords},beforeSend:function(){},success:function(response){window.location=BaseURL+response;}});}}